...
try {
    ...
    console.log("Connected to the Jarvis_Md Whatsapp Bot By Vince")
} catch {
...
setTimeout(() => {
    ...
    global.srepo = 'https://github.com/Vince/Jarvis-Md'
    global.scap = '*ᴘᴏᴡᴇʀᴇᴅ ʙʏ Jarvis-Md* '
    global.sgen = `*GENERATED BY ${name.botname}*`
    global.snam = 'Jarvis-Md'
    global.scate  = `
┏━━⟪⟪ ${mztit} ⟫━◈       
...
┗━━━━━━━━━━◈        `
        ...
        const Void = VoidConnect({
            ...
            browser: ['Jarvis-Md', 'safari', '1.0.0'],
...
*/ //------------------------------------------------------------------------------------
                    ...
                    if (checkinfo) {
                        ...
                        catch(e){console.log(`IsSuhail : ${Zubair} \nGroup Id : ${anu.id}\nerror Returning in Jarvis_Md Group : ` , e)}
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    ...
                    fs.readdirSync(__dirname + "/../plugins").forEach((plugin) => {
                        if (path.extname(plugin).toLowerCase() == ".js") {    require(__dirname + "/../plugins/" + plugin);      }
                    });
                    let version = await getVersionWaweb ()
                    let commits = await DB.syncgit()
                    let Msg = "\n*Perfix*『 " + (prefix === "" ? "null" : prefix) + " 』\n*Mode* " + name.WORKTYPE + "\n*Plugins* " + events.commands.length + "\n";
                    if (commits.total !== 0) {
                      Msg += "┃『 New Update 』\n┃✬ *_Hey " + name.ownername + " Your Bot_*\n┃ *_Have A New Update_*\n┃✬ *Use " + prefix + "updatebot*\n";
                    }
                    ;
                    console.log(Msg);
...
else  
{
	...
	return Void.sendMessage("@s.whatsapp.net" ,{text:"There's An Error To Forward This File"},{ quoted : quote } )
}		
...
app.listen(port, () => console.log(`Jarvis-Md Server listening on port http://localhost:${port}!`));
